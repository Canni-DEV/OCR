...
4. Ejecutar `run.ps1 -ServiceName OcrWorker1 -Port 50051 -WorkingDirectory C:\Code\OCR\ocr-system\src\Ocr.Worker.Paddle` (o solo `.\scripts\run.ps1 -ServiceName OcrWorker1 -Port 50051` si se ejecuta desde `src\Ocr.Worker.Paddle\scripts`, porque el script resuelve la ruta relativa).
...
1. Crear/actualizar el entorno con `.\scripts\setup_python_worker.ps1 -WorkerPath C:\Code\OCR\ocr-system\src\Ocr.Worker.Paddle` (deja el intérprete en `.\.venv\Scripts\python.exe`).
2. Desde la carpeta del worker (por ejemplo `C:\Code\OCR\ocr-system\src\Ocr.Worker.Paddle`), definir variables para la sesión de depuración:
   ```powershell
...
4. Alternativamente, usar `src\Ocr.Worker.Paddle\scripts\run_debug.ps1` para automatizar la creación del virtualenv, instalación de dependencias y ejecución en consola:
   ```powershell
   # Ejecutado desde cualquier carpeta; el script resuelve la ruta relativa al repositorio.
   C:\Code\OCR\ocr-system\src\Ocr.Worker.Paddle\scripts\run_debug.ps1 `
     -Port 50051 -Lang es -Concurrency 1 -DisableAngleCls:$false
   # También permite recrear el entorno: -ForceRecreate
   ```
...
- Instalar con el virtualenv por defecto en la carpeta del worker (ejecutando el script desde `src\Ocr.Worker.Paddle\scripts`, se resuelve la ruta relativa automáticamente):
  ```powershell
  .\scripts\run.ps1 -ServiceName OcrWorker1 -Port 50051 `
    -WorkingDirectory C:\Code\OCR\ocr-system\src\Ocr.Worker.Paddle
  ```
